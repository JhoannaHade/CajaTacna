-- =============================================================
-- Caja Tacna Database Completo (Con Asesores y Préstamos) — Supabase SQL Editor
-- =============================================================
-- INSTRUCCIONES:
...
--     user1@cajatacna.com.pe
--     user2@cajatacna.com.pe
--     user3@cajatacna.com.pe
--   Asesor (clave: 123456):
--     asesor@cajatacna.com.pe
-- =============================================================
...
create table public.solicitudes_prestamo (
  ...
  estado            text not null default 'pendiente' check (estado in ('enviado','pendiente','en_evaluacion','recibido_comite','aprobado','desembolsado','rechazado')),
  
  -- Campos adicionales requeridos por la app móvil
  numero_expediente    text,
  canal                text default 'cliente',
  garantia             text default 'sin garantia',
  seguro_desgravamen   boolean not null default true,
...
begin
  ...
  -- Cuenta corriente en soles de Caja Tacna (saldo 0)
  insert into public.cuentas (user_id, tipo, numero_cuenta, saldo, moneda)
  values (
    new.user_id,
    'Cuenta Corriente',
    '310-' || v_num_base || '-' || lpad(floor(random()*9999)::text, 4, '0'),
    0.00,
    'PEN'
  );

  -- Cuenta de ahorros de Caja Tacna (saldo 0)
  insert into public.cuentas_ahorro (user_id, numero_cuenta, saldo, moneda)
  values (
    new.user_id,
    '311-' || lpad(floor(random()*999999)::text, 6, '0') || '-' || lpad(floor(random()*9999)::text, 4, '0'),
    0.00,
    'PEN'
  );
...
begin
  ...
  delete from auth.users where email = 'user1@cajatacna.com.pe';
  insert into auth.users (
    ...
    'user1@cajatacna.com.pe', hashed_password,
    now(), now(), now(),
  ...
  insert into public.perfiles (
    ...
    uid1, 'Ana Garcia Lopez', 'user1@cajatacna.com.pe',
    'DNI', '45871234',
    '8484', '987654321', 'Av. Bolognesi 456, Tacna', false
  );
  ...
  delete from auth.users where email = 'user2@cajatacna.com.pe';
  insert into auth.users (
    id, instance_id, aud, role,
    email, encrypted_password,
    email_confirmed_at, created_at, updated_at,
    ...
    'authenticated', 'authenticated',
    'user2@cajatacna.com.pe', hashed_password,
    now(), now(), now(),
    '{"provider":"email","providers":["email"]}',
    '{"nombre":"Carlos Ramos"}',
    false, '', '', '', ''
  ...
  insert into public.perfiles (
    ...
    uid2, 'Carlos Ramos Flores', 'user2@cajatacna.com.pe',
    'DNI', '31029847',
    '1751', '912345678', 'Av. San Martin 123, Tacna', false
  );
  ...
  delete from auth.users where email = 'user3@cajatacna.com.pe';
  insert into auth.users (
    id, instance_id, aud, role,
    email, encrypted_password,
    email_confirmed_at, created_at, updated_at,
    ...
    'authenticated', 'authenticated',
    'user3@cajatacna.com.pe', hashed_password,
    now(), now(), now(),
    '{"provider":"email","providers":["email"]}',
    '{"nombre":"Maria Torres"}',
    false, '', '', '', ''
  ...
  insert into public.perfiles (
    ...
    uid3, 'Maria Torres Huaman', 'user3@cajatacna.com.pe',
    'DNI', '72938410',
    '5599', '955123456', 'Calle Francisco Lazo 789, Tacna', false
  );

  insert into public.tarjetas (user_id, numero, tipo, marca, fecha_vencimiento)
  values (uid3, '**** **** **** 5599', 'Debito', 'Visa', '09/29');

  -- ── Usuario Asesor (Asesor Principal Caja Tacna) ───────────
  delete from auth.users where email = 'asesor@cajatacna.com.pe';
  insert into auth.users (
    id, instance_id, aud, role,
    email, encrypted_password,
    email_confirmed_at, created_at, updated_at,
    ...
    'authenticated', 'authenticated',
    'asesor@cajatacna.com.pe', hashed_password,
    now(), now(), now(),
    '{"provider":"email","providers":["email"]}',
    '{"nombre":"Asesor Caja Tacna"}',
    false, '', '', '', ''
  );
  ...
  insert into public.perfiles (
    ...
    uid_asesor, 'Asesor Principal Caja Tacna', 'asesor@cajatacna.com.pe',
    'DNI', '00000001',
    '0000', '999999999', 'Sede Central Francisco Lazo N° 297, Tacna', true
  );
...
begin
  ...
  select id into u1 from auth.users where email = 'user1@cajatacna.com.pe' limit 1;
  select id into u2 from auth.users where email = 'user2@cajatacna.com.pe' limit 1;
  select id into u3 from auth.users where email = 'user3@cajatacna.com.pe' limit 1;
  ...
  insert into public.cuentas (user_id, tipo, numero_cuenta, saldo, moneda)
  values (u1, 'Cuenta Dólares', '310-248301-9812', 1200.00, 'USD')
  ...
  -- Préstamo personal user1 (Caja Tacna)
